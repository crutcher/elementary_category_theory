...
% ═══════════════════════════════════════════════════════════════════════════
\begin{nameorigin}
The \term{presheaf category} $\widehat{\mathcal{C}} = [\mathcal{C}^{\mathrm{op}},
\mathbf{Set}]$ is the ``free cocompletion'' of $\mathcal{C}$ — the
universal cocomplete category containing $\mathcal{C}$. The
``hat'' notation $\widehat{\mathcal{C}}$ is standard in the
Grothendieck-school literature, evoking the analogy with $\widehat{R}$
(the completion of a ring): both add ``ideal'' or ``universal'' missing
constructions. \emph{Cocompletion} = closing under colimits, freely.
Cross-ref Vol IV Ch.~48 for the $\infty$-version.

\term{Density theorem}: every presheaf is canonically a colimit of
representables. The terminology echoes topology (a dense subset of a
space). Mac Lane's standard form; the slogan is that
\emph{representables are dense in the presheaf category}.
\end{nameorigin}
