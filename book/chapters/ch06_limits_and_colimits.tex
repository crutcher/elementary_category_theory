...
% ═══════════════════════════════════════════════════════════════════════════
\begin{nameorigin}
\term{Limit} and \term{colimit} were introduced by \emph{Saunders
Mac Lane} as the general categorical pattern subsuming products,
equalizers, pullbacks, and their duals. The word \emph{limit} echoes
classical analysis (limit of a sequence, limit of a function): a single
object ``approached'' by the whole diagram. The dual \emph{colimit}
(``co-limit'') generalizes coproduct, coequalizer, and pushout — the
``glued-together'' constructions. The pattern is one of the central
unifications of category theory: dozens of seemingly different constructions
across mathematics become instances of one universal-property template.

\term{Diagram} as a functor $D : \mathcal{J} \to \mathcal{C}$ formalizes
``the shape'' of a drawing: $\mathcal{J}$ is the shape-category whose
objects are ``slots'' and whose morphisms are ``required arrows.''
The viewpoint is due to Grothendieck and Mac Lane.
\end{nameorigin}
...
\begin{itemize}
  \item \term{Equalizer}: where two parallel arrows ``become equal'' —
        the universal sub-object on which $f = g$.
  \item \term{Pullback}: from algebraic geometry, where one ``pulls
        back'' a function along a map of varieties; the fibre product
        $X \times_Z Y$.
  \item \term{Pushout} (dual of pullback): from topology / gluing
        constructions; ``pushes out'' along inclusions.
  \item \term{Coequalizer}: the dual of equalizer; the universal
        quotient on which $f = g$.
\end{itemize}
The terminology is mostly from algebraic topology (Eilenberg-Steenrod
1952) and algebraic geometry (Grothendieck school). The category-theoretic
unification subsumes them all under the limit/colimit umbrella.
\end{nameorigin}
...
\begin{exercisesbox}
\begin{qa}
\qaitem{What is a diagram in $\mathcal{C}$, formally?}{A functor $D : \mathcal{J} \to \mathcal{C}$ from a small category $\mathcal{J}$.}
\qaitem{What plays the role of ``the shape''?}{The index category $\mathcal{J}$.}
\qaitem{What is the constant functor $\Delta_X$?}{The functor sending every object to $X$ and every arrow to $\mathrm{id}_X$.}
\qaitem{What is a cone over $D$ with apex $X$?}{A natural transformation $\Delta_X \Rightarrow D$.}
\qaitem{Unpacked, what data does a cone consist of?}{A morphism $\lambda_j : X \to D(j)$ for every $j$, compatible with the arrows of $\mathcal{J}$.}
\qaitem{What is a limit of $D$?}{A universal cone over $D$.}
\qaitem{Why is the limit unique up to unique iso?}{It satisfies a universal property; \S5.4 applies.}
\qaitem{Empty index category: what is the limit?}{The terminal object.}
\qaitem{Singleton index category: what is the limit?}{The unique object that $D$ picks out.}
\qaitem{Discrete two-object index: what is the limit?}{The (binary) product.}
\qaitem{Parallel pair index: what is the limit?}{The equalizer.}
\qaitem{Cospan index: what is the limit?}{The pullback.}
\qaitem{What is a cocone under $D$?}{A natural transformation $D \Rightarrow \Delta_X$.}
\qaitem{What is a colimit?}{A universal cocone.}
\qaitem{Empty index: what is the colimit?}{The initial object.}
\qaitem{Discrete two-object index: colimit?}{The coproduct.}
\qaitem{Parallel pair: colimit?}{The coequalizer.}
\qaitem{Span index: colimit?}{The pushout.}
\qaitem{Why call the colimit ``a glue''?}{It glues the diagram's objects together along the diagram's arrows.}
\qaitem{In $\mathbf{Set}$, how is the pullback realized?}{As $\{(a,b) \in A \times B \mid f(a) = g(b)\}$.}
\qaitem{In $\mathbf{Set}$, how is the equalizer realized?}{As $\{a \in A \mid f(a) = g(a)\}$ with the inclusion into $A$.}
\qaitem{In $\mathbf{Set}$, how is the coequalizer realized?}{As $B$ modulo the equivalence relation generated by $f(a) \sim g(a)$.}
\qaitem{What does ``$\mathcal{C}$ is complete'' mean?}{$\mathcal{C}$ has all small limits.}
\qaitem{What is the minimal checklist for completeness?}{Terminal object, all binary products, all equalizers.}
\qaitem{What is ``$\mathcal{C}$ is cocomplete''?}{Has all small colimits.}
\qaitem{Minimal checklist for cocompleteness?}{Initial object, all binary coproducts, all coequalizers.}
\qaitem{What does ``$F$ preserves limits'' mean?}{$F$ sends limiting cones to limiting cones.}
\qaitem{What is ``$F$ reflects limits''?}{If $F\lambda$ is a limit, then $\lambda$ was already a limit.}
\qaitem{Where are limits computed in $[\mathcal{C}, \mathcal{D}]$?}{Pointwise, in $\mathcal{D}$.}
\qaitem{What is the formula for $(\lim D)(C)$ pointwise?}{$\lim_{j} D(j)(C)$.}
\qaitem{Why is ``pointwise'' so useful?}{It reduces hard calculations in functor categories to easier ones in the target category.}
\qaitem{Are products limits over what shape?}{The discrete category on two (or more) objects.}
\qaitem{Why is duality a labour-saver here?}{We prove a theorem about limits, and get a colimit theorem free by reversing arrows.}
\end{qa}
\end{exercisesbox}
