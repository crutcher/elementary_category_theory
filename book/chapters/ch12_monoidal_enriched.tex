...
% ─────────────────────────────────────────────────────────────────────────────
\begin{nameorigin}
\term{Monoidal category} was introduced by \emph{Jean Bénabou} in 1963.
The term combines \emph{monoid} (a set with an associative
multiplication and unit) with the categorical extension: a monoidal
category is a category whose ``multiplication'' is a tensor product
$\otimes$ on objects (a bifunctor), with a coherent associator and unit
isomorphisms playing the role of associativity and unit laws.

\term{Enriched category} (or \emph{$\mathcal{V}$-category}) was
developed by \emph{Samuel Eilenberg} and \emph{Max Kelly} (1966). The
adjective \emph{enriched} reflects that the hom-objects are richer
than mere sets — they carry $\mathcal{V}$-structure. The terminology
contrasts \emph{enriched} categories with the \emph{ordinary} (= $\mathbf{Set}$-enriched)
categories of Vol I Ch.~2. Major modern foundations (Vol III Ch.~20)
develop enriched CT in depth.

\term{Pentagon} and \term{triangle} (the Mac Lane coherence axioms)
are named for the geometric shapes of the commutative diagrams they
require. Mac Lane's 1963 \emph{Natural Associativity and Commutativity}
proved the pentagon $+$ triangle suffices to imply \emph{all} higher
coherences — the celebrated \emph{Mac Lane coherence theorem}.
\end{nameorigin}
